...
// Yordamchi komponent: To'liq ekran rasm (Zoomable)
const FullscreenImage = ({ src, alt, onClose }) => {
  return (
    ...
    >
      <button 
        className="absolute top-6 right-6 z-50 p-3 rounded-full bg-white/10 text-white/70 hover:text-white hover:bg-white/20 transition-all active:scale-90"
        onClick={(e) => { e.stopPropagation(); onClose(); }}
      >
        <X size={24} />
      </button>
      
      {/* Pan & Zoom xususiyati bilan rasm */}
      <motion.img
        drag
        ...
        src={src}
        alt={alt}
        className="max-w-[95vw] max-h-[90vh] object-contain rounded-2xl cursor-grab active:cursor-grabbing"
        onClick={(e) => e.stopPropagation()} // Rasmni bosganda yopilib ketmasligi uchun
      />
    </motion.div>
  );
};
...
const UserProfileModal = ({ isOpen, onClose, user, role = 'user', isOnline = false }) => {
  ...
  // Modalni yopganda Fullscreen rasm ham yopilishi kerak
  useEffect(() => {
  ...
  // Keshni yo'qotish uchun rasmlar ssilkasi oxiriga timestamp qo'shamiz (Faqat user_updated_at maydoni o'zgarganda)
  // Ammo oddiylik uchun avatar URL oxiriga har doim Date.now() ulab qo'yish kesh muammosini yechaoladi
  ...
  return (
    <>
        ...
        {isOpen && !showFullImage && (
          <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden">
            ...
            <motion.div
              ...
              onClick={handleCloseModal} // Faqat backdrop bosilganda yopiladi
            />
            ...
            <motion.div
              ...
              drag="y"
              dragConstraints={{ top: 0, bottom: 0 }} // Pastga tortilgandagina yopilishi uchun
              dragElastic={0.2}
              onDragEnd={(e, info) => {
              ...
              onClick={(e) => e.stopPropagation()} // Ichkarida bosilganda Modal yopilmasligi uchun
            >
              {/* Drag Indicator (Pill) */}
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pb-6">
                ...
                <div className="relative h-[140px] w-full flex justify-center">
                  ...
                  {/* Floating Avatar (Klik qilsa kattalashadigan) */}
                  <div 
                    className={`absolute -bottom-12 shadow-2xl rounded-full p-1.5 bg-[#1c1c1e]/50 backdrop-blur-xl border border-white/10 ${avatarUrl ? 'cursor-pointer hover:scale-105 transition-transform' : ''}`}
      ...
      {/* Rasm to'liq ekranda ochiladigan qism */}
      <AnimatePresence>
        {showFullImage && avatarUrl && (
          <FullscreenImage src={avatarUrl} alt={fullName} onClose={() => setShowFullImage(false)} />
        )}
      </AnimatePresence>
    </>
  );
