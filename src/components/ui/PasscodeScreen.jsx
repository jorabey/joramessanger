...
const PasscodeScreen = ({ 
  ...
  // Vaqtni hisoblash (Refresh bo'lganda ham ishlaydi)
  useEffect(() => {
    if (lockoutUntil > 0) {
  ...
  const handleKeyPress = useCallback((val) => {
    if (timeLeft > 0) return; // Qulflangan bo'lsa knopkalar ishlamaydi
  ...
  const handleWrongPasscode = () => {
    ...
    // 5 marta xato qilsa 30 soniyaga bloklaymiz
    if (newAttempts >= 5) {
      const lockTime = Date.now() + 30000; // 30 soniya
      ...
      const verifyInput = async () => {
        if (mode === 'verify' || mode === 'remove') {
          if (passcode === savedPasscode) {
            // To'g'ri bo'lsa xotirani tozalaymiz
            localStorage.removeItem('app_failed_attempts');
            localStorage.removeItem('app_lockout_time');
            onSuccess(passcode);
          } else {
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex flex-col items-center justify-center bg-[#000000]/95 backdrop-blur-3xl text-white">
      {onClose && mode !== 'verify' && (
        <button onClick={onClose} className="absolute top-12 right-6 p-2 bg-white/10 rounded-full active:scale-90 transition-transform">
          <X size={24} />
        </button>
      )}
      ...
      <div className="flex flex-col items-center flex-1 w-full max-w-[320px] pt-[15vh]">
        ...
        {timeLeft > 0 ? (
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center mb-8">
            ...
            <h2 className="text-[20px] font-bold text-white mb-2">Ilova qulflangan</h2>
            <p className="text-[15px] text-slate-400 text-center">
              Ko'p marta xato urindingiz.<br/>
              <span className="text-red-400 font-bold text-[18px] block mt-2">{timeLeft} soniya</span> kuting.
            </p>
          </motion.div>
        ) : (
          <>
            ...
            <p className="text-[14px] text-red-400 h-6 font-medium text-center">
              {errorText}
            </p>
              ...
              {Array.from({ length: passcodeLength }).map((_, i) => (
                <motion.div
                  ...
                  animate={{
                    ...
                    backgroundColor: passcode.length > i ? '#ffffff' : 'rgba(255, 255, 255, 0.1)',
                    borderColor: passcode.length > i ? '#ffffff' : 'rgba(255, 255, 255, 0.3)'
                  }}
        ...
        <div className={`grid grid-cols-3 gap-x-6 gap-y-4 mt-auto pb-12 w-full px-6 transition-opacity duration-300 ${timeLeft > 0 ? 'opacity-30 pointer-events-none' : 'opacity-100'}`}>
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
            <button
              ...
              className="w-[75px] h-[75px] rounded-full flex items-center justify-center bg-white/10 hover:bg-white/20 active:bg-white/30 transition-colors mx-auto"
            >
          ...
          <button
            ...
            className="w-[75px] h-[75px] rounded-full flex items-center justify-center text-white/70 hover:bg-white/10 active:bg-white/20 transition-colors disabled:opacity-30 mx-auto"
          >
            <Delete size={28} />
          </button>
        </div>
